library(siMMMulator)

# Step 0: basic parameters
my_variables <- step_0_define_basic_parameters(
  years = 2,
  channels_impressions = c("Facebook", "TV"),
  channels_clicks = c("Search"),
  frequency_of_campaigns = 1,
  true_cvr = c(0.001, 0.002, 0.003),
  revenue_per_conv = 1,
  start_date = "2017/1/1"
)

# Step 1: baseline sales (without ads)
df_baseline <- step_1_create_baseline(
  my_variables = my_variables,
  base_p = 10000,
  trend_p = 0.5,
  temp_var = 2,
  temp_coef_mean = 100,
  temp_coef_sd = 500,
  error_std = 100
)

# Step 2: ad spend per campaign/channel
df_ads_step2 <- step_2_ads_spend(
  my_variables = my_variables,
  campaign_spend_mean = 329000,
  campaign_spend_std = 100000,
  max_min_proportion_on_each_channel = c(0.45, 0.55,   # Facebook
                                          0.10, 0.15)   # TV
  # Search gets the remainder
)

# Step 3: spend -> impressions/clicks
df_ads_step3 <- step_3_generate_media(
  my_variables = my_variables,
  df_ads_step2 = df_ads_step2,
  true_cpm = c(2, 20, NA),
  true_cpc = c(NA, NA, 0.25),
  mean_noisy_cpm_cpc = c(1, 0.05, 0.01),
  std_noisy_cpm_cpc = c(0.01, 0.15, 0.01)
)

# Step 4: noisy CVRs
df_ads_step4 <- step_4_generate_cvr(
  my_variables = my_variables,
  df_ads_step3 = df_ads_step3,
  mean_noisy_cvr = c(0, 0.0001, 0.0002),
  std_noisy_cvr = c(0.001, 0.002, 0.003)
)

# Step 5a: pivot to daily MMM format
df_ads_step5a <- step_5a_pivot_to_mmm_format(
  my_variables = my_variables,
  df_ads_step4 = df_ads_step4
)

# Step 5b: adstock decay
df_ads_step5b <- step_5b_decay(
  my_variables = my_variables,
  df_ads_step5a_before_mmm = df_ads_step5a,
  true_lambda_decay = c(0.1, 0.2, 0.3)
)

# Step 5c: diminishing returns (saturation)
df_ads_step5c <- step_5c_diminishing_returns(
  my_variables = my_variables,
  df_ads_step5b = df_ads_step5b,
  alpha_saturation = c(2, 2, 2),
  gamma_saturation = c(0.1, 0.2, 0.3)
)

# Step 6: conversions
df_ads_step6 <- step_6_calculating_conversions(
  my_variables = my_variables,
  df_ads_step5c = df_ads_step5c
)

# Step 7: merge with baseline
df_ads_step7 <- step_7_expanded_df(
  my_variables = my_variables,
  df_ads_step6 = df_ads_step6,
  df_baseline = df_baseline
)

# Step 8: ground-truth ROI
step_8_calculate_roi(my_variables = my_variables, df_ads_step7 = df_ads_step7)

# Step 9: final clean data frames (daily + monthly)
list_of_df_final <- step_9_final_df(my_variables = my_variables, df_ads_step7 = df_ads_step7)

df_daily   <- list_of_df_final[[1]]
df_monthly <- list_of_df_final[[2]]

cat("\n--- daily df head ---\n")
print(head(df_daily))
cat("\n--- monthly df head ---\n")
print(head(df_monthly))

write.csv(df_daily,   "df_daily.csv",   row.names = FALSE)
write.csv(df_monthly, "df_monthly.csv", row.names = FALSE)
cat("\nSaved df_daily.csv and df_monthly.csv\n")
